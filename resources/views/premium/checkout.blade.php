...
<div class="max-w-4xl mx-auto"
  ...
  <div class="grid md:grid-cols-3 gap-6">
    ...
    <div class="md:col-span-2 space-y-4">
      ...
      <div x-show="step === 1" x-transition class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h3 class="font-bold text-gray-800 mb-5" style="font-family:'Plus Jakarta Sans',sans-serif;">
        ...
        <div class="grid gap-4">
          ...
          <div @click="period = 'monthly'"
            :class="period === 'monthly' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
            class="border-2 rounded-2xl p-5 cursor-pointer transition-all">
            <div class="flex items-center justify-between">
              <div>
                <div class="font-bold text-gray-800">Bulanan</div>
                <div class="text-gray-500 text-sm mt-0.5">Bayar setiap bulan, batalkan kapan saja</div>
              </div>
              <div class="text-right">
                <div class="font-extrabold text-blue-600 text-xl">
                  Rp {{ number_format($planData['price'], 0, ',', '.') }}
                </div>
                <div class="text-gray-400 text-xs">per bulan</div>
              </div>
            </div>
          </div>
          ...
          <div @click="period = 'yearly'"
            :class="period === 'yearly' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="font-bold text-gray-800">Tahunan</div>
              ...
              <div class="text-right">
                ...
                <div class="text-gray-400 text-xs">per tahun</div>
              </div>
            </div>
          </div>
          @endif
      ...
      <div x-show="step === 2" x-transition class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h3 class="font-bold text-gray-800 mb-5" style="font-family:'Plus Jakarta Sans',sans-serif;">
        ...
        <div class="space-y-3">
          ...
          <div>
            <div @click="method = 'bank_transfer'"
              :class="method === 'bank_transfer' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
              class="border-2 rounded-xl p-4 cursor-pointer transition-all">
              <div class="flex items-center gap-3">
                <span class="text-2xl">🏦</span>
                <div class="flex-1">
                  <div class="font-semibold text-gray-800 text-sm">Transfer Bank</div>
                  <div class="text-gray-400 text-xs">Virtual Account BCA, BNI, BRI, Mandiri</div>
                </div>
                <div class="w-5 h-5 rounded-full border-2 transition-all"
                  :class="method === 'bank_transfer' ? 'border-blue-600 bg-blue-600' : 'border-gray-300'">
                </div>
              </div>
              ...
              <div x-show="method === 'bank_transfer'" x-transition class="mt-4 flex flex-wrap gap-2">
                ...
                <button type="button" @click.stop="bank = '{{ $b }}'"
                  :class="bank === '{{ $b }}' ? 'border-blue-600 bg-blue-50 text-blue-700 font-semibold' : 'border-gray-200 text-gray-600'"
          ...
          <div>
            <div @click="method = 'ewallet'"
              :class="method === 'ewallet' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
              class="border-2 rounded-xl p-4 cursor-pointer transition-all">
              <div class="flex items-center gap-3">
                <span class="text-2xl">📱</span>
                <div class="flex-1">
                  <div class="font-semibold text-gray-800 text-sm">E-Wallet</div>
                  <div class="text-gray-400 text-xs">GoPay, OVO, DANA, ShopeePay</div>
                </div>
                <div class="w-5 h-5 rounded-full border-2 transition-all"
                  :class="method === 'ewallet' ? 'border-blue-600 bg-blue-600' : 'border-gray-300'">
                </div>
              </div>
              ...
              <div x-show="method === 'ewallet'" x-transition class="mt-4 flex flex-wrap gap-2">
                ...
                <button type="button" @click.stop="wallet = '{{ $w }}'"
                  :class="wallet === '{{ $w }}' ? 'border-blue-600 bg-blue-50 text-blue-700 font-semibold' : 'border-gray-200 text-gray-600'"
                  class="border-2 px-4 py-2 rounded-xl text-sm transition">
                  {{ $w }}
                </button>
                @endforeach
          ...
          <div @click="method = 'qris'"
            :class="method === 'qris' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
            class="border-2 rounded-xl p-4 cursor-pointer transition-all">
            <div class="flex items-center gap-3">
              <span class="text-2xl">📷</span>
              <div class="flex-1">
                <div class="font-semibold text-gray-800 text-sm">QRIS</div>
                <div class="text-gray-400 text-xs">Scan QR dari semua aplikasi dompet digital</div>
              </div>
              <div class="w-5 h-5 rounded-full border-2 transition-all"
                :class="method === 'qris' ? 'border-blue-600 bg-blue-600' : 'border-gray-300'">
              </div>
            </div>
          </div>
          ...
          <div @click="method = 'credit_card'"
            :class="method === 'credit_card' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'"
            class="border-2 rounded-xl p-4 cursor-pointer transition-all">
            <div class="flex items-center gap-3">
              <span class="text-2xl">💳</span>
              <div class="flex-1">
                <div class="font-semibold text-gray-800 text-sm">Kartu Kredit / Debit</div>
                <div class="text-gray-400 text-xs">Visa, Mastercard, semua bank</div>
              </div>
              <div class="w-5 h-5 rounded-full border-2 transition-all"
                :class="method === 'credit_card' ? 'border-blue-600 bg-blue-600' : 'border-gray-300'">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-3 mt-6">
          ...
          <button @click="method ? step = 3 : null"
            :disabled="!method"
            class="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white font-semibold py-3 rounded-xl transition text-sm">
            Lanjut — Konfirmasi →
          </button>
        </div>
      </div>
      ...
      {{-- STEP 3: Konfirmasi --}}
      <div x-show="step === 3" x-transition class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h3 class="font-bold text-gray-800 mb-5" style="font-family:'Plus Jakarta Sans',sans-serif;">
          Konfirmasi Pesanan
        </h3>
        ...
        <div class="bg-gray-50 rounded-xl p-4 space-y-3 mb-5 text-sm">
          <div class="flex justify-between">
            ...
            <span class="font-semibold">{{ $planData['name'] }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">Periode</span>
            <span class="font-semibold" x-text="period === 'yearly' ? 'Tahunan' : 'Bulanan'"></span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">Metode</span>
            <span class="font-semibold" x-text="method === 'bank_transfer' ? '🏦 Transfer Bank (' + bank + ')' : method === 'ewallet' ? '📱 ' + wallet : method === 'qris' ? '📷 QRIS' : '💳 Kartu Kredit'"></span>
          </div>
          <div x-show="promoApplied" class="flex justify-between text-green-600">
            <span>Diskon Promo (SMARTKA10)</span>
            <span class="font-semibold" x-text="'- ' + formatRp(promoDiscount)"></span>
          </div>
          <div class="border-t border-gray-200 pt-3 flex justify-between">
            <span class="font-bold text-gray-800">Total Bayar</span>
            <span class="font-extrabold text-blue-600 text-lg" x-text="formatRp(finalPrice)"></span>
          </div>
        </div>
        ...
        <div class="flex gap-2 mb-5">
          ...
          <button @click="applyPromo()"
            class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-4 py-2.5 rounded-xl text-sm transition">
            Pakai
          </button>
        </div>
    ...
    <div class="space-y-4">
      <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-24">
        <h4 class="font-bold text-gray-800 mb-4 text-sm">Ringkasan Pesanan</h4>

        {{-- Paket info --}}
        <div class="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
          <div class="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">⭐</div>
          <div>
            <div class="font-semibold text-gray-800 text-sm">{{ $planData['name'] }}</div>
            <div class="text-gray-400 text-xs" x-text="period === 'yearly' ? 'Tahunan' : 'Bulanan'"></div>
          </div>
        </div>
        ...
        <div class="space-y-2 text-sm mb-4">
          ...
          <div class="flex justify-between font-bold text-gray-800 pt-2 border-t border-gray-100">
            <span>Total</span>
            <span class="text-blue-600" x-text="formatRp(finalPrice)"></span>
          </div>
        </div>
